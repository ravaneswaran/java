...
public class BullishThreeOutsideUpDetector extends AbstractPatternDetector {

    public BullishThreeOutsideUpDetector() {
        super(3);
    }
    ...
    public boolean detect(List<Candle> candles) {
        if (null != candles && candles.size() >= this.minimumCandles) {
            return BullishThreeOutsideUpDetector.isThreeOutsideUp(candles.get(0), candles.get(1), candles.get(2));
        } else {
            return false;
        }
    }
