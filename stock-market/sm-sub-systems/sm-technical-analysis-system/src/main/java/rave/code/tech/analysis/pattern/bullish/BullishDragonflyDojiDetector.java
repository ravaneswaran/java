...
public class BullishDragonflyDojiDetector extends AbstractPatternDetector {

    public BullishDragonflyDojiDetector() {
        super(1);
    }
    ...
    public boolean detect(List<Candle> candles) {
        if (null != candles && candles.size() >= this.minimumCandles) {
            return BullishDragonflyDojiDetector.isDragonflyDoji(candles.get(0));
        } else {
            return false;
        }
    }
