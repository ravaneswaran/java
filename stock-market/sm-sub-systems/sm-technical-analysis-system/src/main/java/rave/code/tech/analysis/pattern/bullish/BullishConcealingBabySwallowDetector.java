...
public class BullishConcealingBabySwallowDetector extends AbstractPatternDetector {

    private static final double SHADOW_TOLERANCE = 0.05;

    public BullishConcealingBabySwallowDetector() {
        super(4);
    }
    ...
    public static boolean isConcealingBabySwallow(Candle c1, Candle c2, Candle c3, Candle c4) {
        ...
        boolean gapDownSecond = c2.getOpen() < c1.getClose();
        ...
        boolean gapDownThird = c3.getHigh() < c2.getLow();

        // Fourth candle bearish
        boolean fourthBearish = c4.isBearish();

        // Fourth candle opens above third body
        boolean opensAboveThirdBody = c4.getOpen() > Math.max(c3.getOpen(), c3.getClose());

        // Fourth candle engulfs third candle range
        boolean engulfsThirdRange = c4.getHigh() > c3.getHigh() && c4.getLow() < c3.getLow();
    ...
    public boolean detect(List<Candle> candles) {
        if (null != candles && candles.size() >= this.minimumCandles) {
            return BullishConcealingBabySwallowDetector.isConcealingBabySwallow(candles.get(0), candles.get(1), candles.get(2), candles.get(3));
        } else {
            return false;
        }
    }
