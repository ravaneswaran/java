package rave.code.tech.analysis.pattern.bullish;

import rave.code.tech.analysis.pattern.AbstractPattern;
import rave.code.tech.analysis.Candle;

import java.util.List;

public class BullishThreeWhiteSoldiersPattern extends AbstractPattern {

    public BullishThreeWhiteSoldiersPattern() {
        super(3);
    }

    public static boolean isThreeWhiteSoldiers(
            Candle c1,
            Candle c2,
            Candle c3) {

        // All candles bullish
        boolean bullishCandles =
                c1.isBullish() &&
                        c2.isBullish() &&
                        c3.isBullish();

        // Higher closes
        boolean higherCloses =
                c1.getClosePrice() < c2.getClosePrice() &&
                        c2.getClosePrice() < c3.getClosePrice();

        // Opens inside previous body
        boolean properOpens =
                c2.getOpenPrice() > c1.getOpenPrice() &&
                        c2.getOpenPrice() < c1.getClosePrice() &&

                        c3.getOpenPrice() > c2.getOpenPrice() &&
                        c3.getOpenPrice() < c2.getClosePrice();

        // Strong candle bodies
        boolean strongBodies =
                c1.bodySize() > 0 &&
                        c2.bodySize() > 0 &&
                        c3.bodySize() > 0;

        return bullishCandles &&
                higherCloses &&
                properOpens &&
                strongBodies;
    }

    @Override
    public String getName() {
        return "Bullish Three White Soldiers";
    }

    @Override
    public boolean matches(List<Candle> candles) {
        return false;
    }

    @Override
    public boolean detect(List<Candle> candles) {
        if (null != candles && candles.size() >= this.minimumCandles) {
            return BullishThreeWhiteSoldiersPattern.isThreeWhiteSoldiers(candles.get(0), candles.get(1), candles.get(2));
        } else {
            return false;
        }
    }
}
