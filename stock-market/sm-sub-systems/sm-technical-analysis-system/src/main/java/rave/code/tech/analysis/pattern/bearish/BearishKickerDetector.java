...
public class BearishKickerDetector extends AbstractPatternDetector {

    public BearishKickerDetector() {
        super(2);
    }
    ...
    public static boolean isKicker(Candle c1, Candle c2) {
        ...
        // Gap down open
        boolean gapDown = c2.getOpen() < c1.getOpen();
    ...
    public boolean detect(List<Candle> candles) {
        if (null != candles && candles.size() >= this.minimumCandles) {
            return BearishKickerDetector.isKicker(candles.get(0), candles.get(1));
        } else {
            return false;
        }
    }
