...
public class BullishRisingThreeMethodsDetector extends AbstractPatternDetector {

    public BullishRisingThreeMethodsDetector() {
        super(5);
    }
    ...
    public boolean detect(List<Candle> candles) {
        if (null != candles && candles.size() >= this.minimumCandles) {
            return BullishRisingThreeMethodsDetector.isRisingThreeMethods(candles.get(0), candles.get(1), candles.get(2), candles.get(3), candles.get(4));
        } else {
            return false;
        }
    }
